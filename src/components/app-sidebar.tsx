...
import { Building2, Home, Users, FileText, Receipt, Wrench, BarChart3, Settings, LogOut } from "lucide-react";
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="border-b border-sidebar-border">
        <Link to="/" className="flex items-center gap-2 p-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-md bg-accent text-accent-foreground"><Building2 className="h-4 w-4"/></div>
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel>{role === "tenant" ? "Tenant" : role === "owner" ? "Owner" : "Manage"}</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {items.map((it) => (
